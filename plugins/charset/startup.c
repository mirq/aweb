...
static struct SignalSemaphore charsema;
...
)
{
   ...
   UBYTE  *buf, *wrongtxt, *failtxt, *urltxt;
   UWORD  version, revision;
   static BOOL awebok=FALSE;
   static BOOL warned=FALSE;
   static BOOL warnold=FALSE;

#define MINVERSION "3.5.09"
#define WARNTITLE "awebcharset.awebplugin"
#define WARNINTRO "This awebcharset.awebplugin need at least AWeb " MINVERSION "\n"\
                  "Please download and install the latest AWeb Version from:\n"\
                  "http://aweb.sunsite.dk\n\n"
#define WARNWRONG "This AWeb Version is not working with this Plugin!"
#define WARNTOOLD "The Filterfunction is now disabled!"

#define CODESETLIB "codesets.library"
#define CODESETVER 6
#define CODESETREV 2
#define CODESETLEN 32 /* length of Name + Versions + Revisions */
   ...
   if(!AwebSupportBase)
   {
      ...
      if(!AwebSupportBase)
      {
         // no AwebSupportBase (Version 2) - we must run on an *very* old Version
         // inform the User once about that
         if(warnold==FALSE)
         {
            if(IntuitionBase=(struct IntuitionBase *) OpenLibrary("intuition.library", 36))
            {
               ...
               es.es_Title=WARNTITLE;
               es.es_TextFormat=WARNINTRO WARNWRONG;
...
#endif
         if(strncmp(Awebversion(),MINVERSION,strlen(MINVERSION))<0)
         {
            // we must run on an old Version - don't load codesets.library to keep AWeb usable.
            // inform the User about that. Use catalog String for OK Button.
            Syncrequest(WARNTITLE, WARNINTRO WARNTOOLD, Awebstr(MSG_ABOUT_OK));
         }
         else
         {
            awebok=TRUE;
   ...
   if(awebok && !CodesetsBase) // no check for warned here so the User can add the Lib without restart AWeb
   {
      CodesetsBase=OpenLibrary(CODESETLIB,CODESETVER);
      if(CodesetsBase)
      {
         version=CodesetsBase->lib_Version;
         revision=CodesetsBase->lib_Revision;
         if(CodesetsBase->lib_Version == CODESETVER && revision < CODESETREV)
         {
            CloseLibrary(CodesetsBase);
            CodesetsBase = NULL;

            // Wrong codesets revision - build a requester to inform the user once about that
            if(Agetattr(Aweb(),AOAPP_Screenvalid) && warned==FALSE)
            {
               wrongtxt=Awebstr(MSG_CHARSET_CODESETWRONG);
               urltxt=Awebstr(MSG_CHARSET_CODESETURL);

               if(buf=Allocmem(strlen(wrongtxt) + strlen(urltxt) + CODESETLEN,MEMF_CLEAR))
               {
                  sprintf(buf, wrongtxt, CODESETLIB, version, revision, CODESETVER, CODESETREV);
                  strcat(buf, urltxt);
                  Syncrequest(Awebstr(MSG_CHARSET_CODESETTITLE),buf,Awebstr(MSG_CHARSET_OK));
                  warned=TRUE;
                  Freemem(buf);
               }
            }
      ...
      if(!CodesetsBase && warned==FALSE)
      {
         // No codesets library - build a requester to inform the user once about that
         if(Agetattr(Aweb(),AOAPP_Screenvalid) && warned==FALSE)
         {
            failtxt=Awebstr(MSG_CHARSET_CODESETFAIL);
            urltxt=Awebstr(MSG_CHARSET_CODESETURL);
            if(buf=Allocmem(strlen(failtxt)+ strlen(urltxt)+ CODESETLEN, MEMF_CLEAR))
            {
               sprintf(buf, failtxt, CODESETLIB, CODESETVER, CODESETREV);
               strcat(buf, urltxt);
               Syncrequest(Awebstr(MSG_CHARSET_CODESETTITLE),buf,Awebstr(MSG_CHARSET_OK));
               warned=TRUE;
               Freemem(buf);
...
)
{
  ...
  Pluginlibbase->lib_OpenCnt--;
   if(Pluginlibbase->lib_OpenCnt==0)
   {
      ...
      {
         result = Real_Expungelib(LIBMAN_NAME);
      }
      /* close the AwebSupport Library here because it's used in Expungepluginlib */
#if defined(__amigaos4__)
      if(IAwebSupport)
      {
