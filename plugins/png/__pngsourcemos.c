...
{  struct Sourcedriver sourcedriver;/* Or superclass object instance */
   ...
   UBYTE *argb;                     /* The ARGB array for the image */
...
{  struct Datablock *current;       /* The current datablock */
   ...
   /* Output fields: */
   UBYTE *argb;                     /* ARGB array to be filled */
...
{  void *p=AllocMem(size+4, MEMF_ANY);
   if(p)
   {  *(ULONG *)p=size+4;
...
{  png_structp png=NULL;
   ...
   {  if(!setjmp(png->jmpbuf))
      {  /* Normal fallthrough */
         ...
         decoder->argb = AllocVec(decoder->width * 4 * decoder->height, MEMF_ANY);

         /* Save our bitmap and dimensions. */
         ObtainSemaphore(&decoder->source->sema);
         decoder->source->argb=decoder->argb;
         decoder->source->width=decoder->width;
         decoder->source->height=decoder->height;
         decoder->source->alpha=decoder->flags & DECOF_TRANSPARENT;
         ReleaseSemaphore(&decoder->source->sema);
            ...
            for(y=0;y<decoder->height;y++)
            {
               ...
               switch (colortype)
               {
                  ...
                  case PNG_COLOR_TYPE_GRAY_ALPHA:
                     for (x = 0; x < width; x++)
                     {
                        buf[4*x+0] = buffer[2*x+1];
                        buf[4*x+1] = buffer[2*x+0];
                        buf[4*x+2] = buffer[2*x+0];
                        buf[4*x+3] = buffer[2*x+0];
                     }
                     break;
                  case PNG_COLOR_TYPE_PALETTE:
                     ...
                     {
                        pen=buffer[x];
                        buf[4*x+0]=(trans && pen<ntrans) ? trans[pen] : 0xff;
                        buf[4*x+1]=palette[pen].red;
                        buf[4*x+2]=palette[pen].green;
                        buf[4*x+3]=palette[pen].blue;
                     }
                     break;
                   case PNG_COLOR_TYPE_RGB:
                     ...
                     {
                        buf[4*x+0]=0xff;
                        buf[4*x+1]=buffer[3*x+0];
                        buf[4*x+2]=buffer[3*x+1];
                        buf[4*x+3]=buffer[3*x+2];
                     }
                     break;
                   case PNG_COLOR_TYPE_RGB_ALPHA:
                                                        for (x = 0; x < width; x++)
                                                        {
                                                                buf[x*4+0] = buffer[x*4+3];
                                                                buf[x*4+1] = buffer[x*4+0];
                                                                buf[x*4+2] = buffer[x*4+1];
                                                                buf[x*4+3] = buffer[x*4+2];
                                                        }
                     break;
               }
...
{
   Anotifyset(ps->source,AOPNG_PixelArray,NULL,TAG_END);
      ...
      {  case AOSDV_Source:
         ...
         case AOSDV_Displayed:
            ...
            {  /* We are becoming displayed. Start our decoder task if
               ...
               if(ps->data.first->next && !ps->argb && !ps->task)
               {  Startdecoder(ps);
               }
            }
            else
...
{  struct TagItem *tag,*tstate;
   ...
   if(data && datalength)
   {  struct Datablock *db;
      if(db=AllocVec(sizeof(struct Datablock),MEMF_CLEAR))
